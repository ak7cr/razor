...
export async function runHeuristicBuyer(session: BuyerSession, mission: string): Promise<void> {
  session.emitEvent('agent.thinking', { text: '[heuristic planner] Parsing mission for products & budget…' });

  const budget = parseBudget(mission);
  if (Number.isFinite(budget)) {
    session.emitEvent('agent.thinking', {
      text: `[heuristic planner] Budget detected: ${formatInr(budget * 100)}.`,
    });
  }

  const keywords = extractKeywords(mission);
  if (keywords.length === 0) {
    session.emitEvent('agent.thinking', { text: 'Could not extract any product keywords from the mission.' });
    return;
  }
  session.emitEvent('agent.thinking', { text: `[heuristic planner] Keywords: ${keywords.join(', ')}` });
  ...
  for (const kw of keywords) {
    ...
    if (hits.length === 0) {
      session.emitEvent('agent.thinking', { text: `[heuristic planner] No match for "${kw}".` });
      continue;
    }
    const pick = hits[0]!;
    if (Number.isFinite(budget) && session.cart.totalPaise + pick.pricePaise > budget * 100) {
      session.emitEvent('agent.thinking', {
        text: `[heuristic planner] Skipping ${pick.name} (${formatInr(pick.pricePaise)}) — over budget.`,
      });
      continue;
    }
    const reasoning = `Mission asked for "${kw}". Best match: ${pick.name} (${pick.id}) at ${formatInr(pick.pricePaise)}, ${pick.stock} in stock.`;
    session.addToCart(pick.id, 1, reasoning);
    added.push(pick.id);
  }

  if (session.cart.lines.length === 0) {
    session.emitEvent('agent.thinking', { text: '[heuristic planner] No buyable items matched the mission.' });
    return;
  }

  session.emitEvent('agent.thinking', { text: '[heuristic planner] Cart complete — proposing the order (gated).' });
